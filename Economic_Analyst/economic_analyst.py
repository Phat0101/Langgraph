...
from backend.routers.Economic_Analyst.state import (
    # from state import (
    ResearchState,
    ResearchStateInput,
    ResearchStateOutput,
...
from backend.routers.Economic_Analyst.prompts import (
    # from prompts import (
...
async def tavily_search_async(query: str) -> str:
    ...
    try:
        ...
        return json.dumps(
            {"error": f"Error performing search: {str(e)}"}, ensure_ascii=False
        )
...
def research_planner(state: ResearchState):
    ...
    # Strictly limit the number of search queries to 3 to avoid halucination
...
async def analyst(state: AnalystState, config: RunnableConfig):
    # Search and analyze
    ...
    analyst = model.with_structured_output(EconomicData)
    analysis_prompt = ANALYSIS_PROMPT.format(
        query=state.search_query, formatted_results=formatted_results
    )
    ...
    summary_prompt = SUMMARY_PROMPT.format(
        current_summary=latest_summary, analysis=formatted_results
    )
...
def should_continue_research(state: AnalystState) -> Literal["reflect", "end_analysis"]:
    ...
    ):  # Maximum 2 or 3 iterations per query, can be adjusted
        return "reflect"
    return "end_analysis"
...
def reflect_on_research(state: AnalystState):
    ...
    reflection_prompt = REFLECTION_PROMPT.format(
        query=state.search_query, current_summary=current_summary
    )
    ...
    if reflection.sufficient:
        ...
        }  # Set to max loop count to end analysis
    else:
...
def aggregate_analyses(state: ResearchState) -> Dict:
    ...
    combined_analysis = EconomicData(
        overview=state.completed_analyses[-1].overview,
        global_economics=state.completed_analyses[-1].global_economics,
        domestic_economics=state.completed_analyses[-1].domestic_economics,
        industry_economics=state.completed_analyses[-1].industry_economics,
        risks=[
            risk for analysis in state.completed_analyses for risk in analysis.risks
        ],
        opportunities=[
            opportunity
        ...
        scenarios=state.completed_analyses[-1].scenarios,
        recommendations=state.completed_analyses[-1].recommendations,
    )
    ...
    combined_summary = model.invoke(
        [
            HumanMessage(
                content=COMBINE_SUMMARIES_PROMPT.format(
    ...
    return {
        ...
        "sources_gathered": [
            f"* {source}" for source in all_sources if source
...
def finalize_report(state: ResearchState):
    ...
    formatted_sources = "\n".join(
        source.strip() for source in state.sources_gathered if source.strip()
    )

    final_summary = (
        state.running_summaries[-1]
        if state.running_summaries
    ...
    return {"final_economic_report": [report], "completed_analyses": state.completed_analyses}


# Parallel analyst_workflow
analyst_workflow = StateGraph(AnalystState, output=AnalystOutputState)
analyst_workflow.add_node("analyst", analyst)
...
analyst_workflow.add_conditional_edges(
    ...
    {"reflect": "reflect", "end_analysis": "end_analysis"},
)
...
# @router.get("/analyse_stock/{stock}")
# async def analyze_industry(stock: str):
#     """Analyse an industry based on the given topic."""
#     try:
